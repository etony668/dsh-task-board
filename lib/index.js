...
function emptyBoard(project) {
  return { project_path: project, project_name: projectName(project), revision: 0, updated_at_ms: 0, tasks: [], layout: { positions: {}, layers: {} }, seen_revision: 0 }
}
...
function ops() {
  return {
    ...
    revision: (args) => withLock(normalizeProject(args.projectPath), async () => {
      const board = await loadBoard(normalizeProject(args.projectPath))
      return { revision: board.revision, updated_at_ms: board.updated_at_ms, project_path: board.project_path, project_name: board.project_name, unread: board.revision > (board.seen_revision || 0) }
    }),

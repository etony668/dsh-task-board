	...
	factory: (require) => {
		...
		function TaskBoardView(props) {
			...
			React.useEffect(function () {
				...
				function stop() {
					...
					try {
						fetch('/api/task-board', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ op: 'layoutSave', projectPath: pp, layout: { positions: positionsRef.current, layers: layersRef.current } }) });
		...
		async function tbPollRevision() {
			...
			try {
				...
				if (data.unread && !tbViewActive) {
					tbUnread = 'red'; tbUpdateDot();
				} else if (!data.unread) {
					if (tbUnread === 'red') { tbUnread = ''; tbUpdateDot(); }
				}
			} catch (e) { }
		}

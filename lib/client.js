...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		var TB_CSS = [
			...
			'.dsh-tb-card-temporary { min-height:112px; border-style:dashed; background:linear-gradient(color-mix(in srgb, #e7a84b 20%, transparent), color-mix(in srgb, #e7a84b 20%, transparent)), var(--tb-panel-bg, var(--dsw-alias-bg-layer-1)); }',
		...
		function TaskBoardView(props) {
			...
			React.useEffect(function () {
				...
				function stop() {
					var had = !!dragRef.current;
					dragRef.current = null;
					var pp = projectPathRef.current;
					if (!had || !pp) {
						try { fetch('/api/task-board', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ op: 'diag', msg: 'stop-noop had=' + had }) }); } catch (e0) { }
						return;
					}
					try {
						fetch('/api/task-board', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ op: 'layoutSave', projectPath: pp, layout: { positions: positions, layers: layers } }) });
						fetch('/api/task-board', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ op: 'diag', msg: 'stop-saved keys=' + Object.keys(positions).length }) });
					} catch (e) { }
				}
		...
		async function tbPollRevision() {
			...
			try {
				...
				if (tbLastRev === null) {
					try {
						var seenR = window.localStorage.getItem('dsh-taskboard-seen-rev:' + (data.project_path || ''));
						if (seenR) tbLastRev = Number(seenR);
					} catch (e) { }
					if (tbLastRev === null) tbLastRev = rev;
					if (tbLastRev !== rev && !tbViewActive) { tbUnread = 'red'; tbUpdateDot(); }
					return;
				}
				if (rev !== tbLastRev) {
					if (!tbViewActive) {
						tbUnread = 'red'; tbUpdateDot();
						try { fetch('/api/task-board', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ op: 'diag', msg: 'unread-red rev=' + rev + ' last=' + tbLastRev + ' view=' + tbViewActive }) }); } catch (e2) { }
					}
					tbLastRev = rev;
				}
			} catch (e) { }
		...
		function apply(ctx) {
			try {
				...
				if (bodyBg && bodyBg !== 'transparent') document.documentElement.style.setProperty('--tb-panel-bg', bodyBg);
				else {
					var dark = typeof window.matchMedia === 'function' ? window.matchMedia('(prefers-color-scheme: dark)').matches : false;
					document.documentElement.style.setProperty('--tb-panel-bg', dark ? '#232324' : '#ffffff');
				}
			} catch (e) { }

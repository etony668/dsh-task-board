	...
	factory: (require) => {
		...
		function tbUpdateDot() {
			try {
				...
				var nodes = header.querySelectorAll('button[role="tab"]');
				var base = t('tabLabel');
		...
		async function tbPollRevision() {
			if (!tbProjectPath || typeof fetch !== 'function') return;
			try {
				var resp = await fetch('/api/task-board', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ op: 'revision', projectPath: tbProjectPath }) });
				if (!resp.ok) return;
		...
		function apply(ctx) {
			...
			var tbPollTimer = setInterval(function () { tbPollRevision(); }, 2500);
			ctx.effect(function () { clearInterval(tbPollTimer); }, 'dsh-task-board: unread-poll');

	...
	factory: (require) => {
		...
		function tbUpdateDot() {
			try {
				...
				var nodes = root.querySelectorAll('button, [role="tab"]');
				var base = t('tabLabel');
				var found = 0;
				for (var i = 0; i < nodes.length; i += 1) {
					var n = nodes[i];
					if (String(n.textContent || '').indexOf(base) === -1) continue;
					n.textContent = base + tbDotSuffix();
					found = 1;
					break;
				}
			} catch (e) { }
		...
		async function tbPollRevision() {
			...
			try {
				...
				try { document.title = 'TB:' + tbUnread + ':r' + tbLastRev + '>' + rev + (tbViewActive ? ':in' : ':out'); } catch (e) { }
				try {
					var matchTabs = 0, header2 = document.querySelector('[data-slot="conversation.session.header"]') || document;
					var tablist2 = header2.querySelector('[role="tablist"]');
					var tabsEls = (tablist2 || header2).querySelectorAll('button, [role="tab"]');
					for (var ti = 0; ti < tabsEls.length; ti += 1) { if (String(tabsEls[ti].textContent || '').indexOf(t('tabLabel')) !== -1) matchTabs += 1; }
				} catch (e) { }

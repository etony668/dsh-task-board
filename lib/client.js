...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		var TB_CSS = [
			...
			'.dsh-tb-panel { position:absolute; z-index:2; padding:12px; border:1px solid var(--dsw-alias-border-l1); border-radius:13px; background:color-mix(in srgb, var(--dsw-alias-bg-layer-1) 97%, transparent); box-shadow:0 12px 34px rgba(0,0,0,.35); }',
			'.dsh-tb-main-panel, .dsh-tb-child-panel { background:color-mix(in srgb, var(--dsw-alias-bg-layer-1) 99%, transparent); }',
			'.dsh-tb-child-panel.dsh-tb-collapsed { padding-bottom:0; }',
			'.dsh-tb-links { position:absolute; inset:0; z-index:0; overflow:visible; pointer-events:none; }',

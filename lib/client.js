...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		var TB_CSS = [
			...
			'.dsh-tb-card { min-height:138px; padding:10px; border:1px solid var(--dsw-alias-border-l1); border-radius:10px; background:color-mix(in srgb, var(--dsw-alias-bg-layer-1) 90%, var(--dsw-alias-brand-primary)); box-shadow:0 5px 16px rgba(0,0,0,.28); transition:border-color 120ms ease, box-shadow 120ms ease, transform 120ms ease; }',
			...
			'.dsh-tb-card.dsh-tb-completed { border-color:var(--dsw-alias-state-success-primary); background:color-mix(in srgb, var(--dsw-alias-bg-layer-1) 90%, var(--dsw-alias-state-success-primary)); }',
			'.dsh-tb-card-temporary { min-height:112px; border-style:dashed; background:color-mix(in srgb, var(--dsw-alias-bg-layer-1) 90%, #e7a84b); }',
		...
		function TaskBoardView(props) {
			...
			React.useEffect(function () {
				tbProjectPath = projectPath || null;
				if (tbLastRev === null && projectPath) {
					tbApi('revision', { projectPath: projectPath }).then(function (r) { if (r && typeof r.revision === 'number') tbLastRev = r.revision; }).catch(function () { });
				}
				if (tbUnread === 'red') { tbUnread = 'green'; tbUpdateDot(); }
				return function () { /* keep tbProjectPath: polling must survive while the view is closed */ };
			}, [projectPath]);
